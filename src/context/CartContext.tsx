...
import { getDbCart, syncDbCart, clearDbCart } from "@/actions/db-cart-actions";
...
export function CartProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (status === "authenticated") {
      const timer = setTimeout(() => {
        syncDbCart(cart.map(i => ({ productId: i.productId, quantity: i.quantity })));
      }, 500);
  ...
  const clearCart = (silent: boolean = false) => {
    ...
    if (status === "authenticated") clearDbCart();
