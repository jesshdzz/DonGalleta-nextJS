...
interface ImageUploadProps {
  ...
  onRemove: () => void;
  value: string;
  endpoint?: "productImage" | "bannerImage";
  onUploadBegin?: () => void;
}
...
export function ImageUpload({
  ...
  return (
    <UploadDropzone
      ...
      appearance={{
        container:
          "border-border bg-muted/20 border-2 border-dashed rounded-lg p-10 cursor-pointer hover:bg-muted/40 transition-colors",
        label: "text-primary font-serif font-bold text-2xl h-auto",
        allowedContent: "text-muted-foreground mt-2 h-auto",
        button:
          "bg-primary text-primary-foreground hover:bg-primary/90 mt-4 px-6 py-2 rounded-md font-medium text-sm transition-colors ut-uploading:cursor-not-allowed ut-uploading:bg-primary/50 after:bg-primary",
